...
describe('VideoManagement calculatePublishedAfter', () => {
  ...
  it('should return an ISO string for pastHour approximately one hour ago', () => {
    ...
    expect(resultTimestamp).toBeGreaterThanOrEqual(expectedTimestamp - ALLOWED_DRIFT_MS);
    ...
    expect(resultTimestamp).toBeLessThanOrEqual(expectedTimestamp + ALLOWED_DRIFT_MS);
  });
  ...
  it('should return an empty string for an invalid recency value', () => {
    const result = videoManagement.calculatePublishedAfter('invalidRecencyValue');
    expect(result).toBe('');
  });
